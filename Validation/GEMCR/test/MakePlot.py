  ...
  for hist in key_list :
    if hist.find("dafdsadsfatrack_") != -1 :
      ...
      for x in [1,3,5,7,9]:
        ...
        tmp2 = tmp2.Clone("chamber_%d_layer_1_ch_efficiency"%x)
        tmp2.Divide(tmp1)
        tmp2.SetTitle("Chamber %d layer 1 Efficiency"%x)
        draw_occ(oDir, tmp2)
        tmp1 = d1.Get("chamber_%d_layer_2_tr_eff"%x)
        tmp2 = d1.Get("chamber_%d_layer_2_ch_eff"%x)
        tmp2 = tmp2.Clone("chamber_%d_layer_2_ch_efficiency"%x)
        tmp2.Divide(tmp1)
        tmp2.SetTitle("Chamber %d layer 2 Efficiency"%x)
        draw_occ(oDir, tmp2)
    else :
      draw_occ( oDir, d1.Get(hist) )

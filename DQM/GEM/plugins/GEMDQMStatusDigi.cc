...
void GEMDQMStatusDigi::analyze(edm::Event const &event, edm::EventSetup const &eventSetup) {
  ...
  for (GEMVfatStatusDigiCollection::DigiRangeIterator vfatIt = gemVFAT->begin(); vfatIt != gemVFAT->end(); ++vfatIt) {
    ...
    if ( listTimeStore_.find((UInt_t)chIdStatus) == listTimeStore_.end() ) {
      edm::LogError("BadGeometry") << "Wrong detId which is not in the current geometry (VFAT status): "
        << gemchId << std::endl;
      continue;
    }
    ...
    for (auto vfatStat = range.first; vfatStat != range.second; ++vfatStat) {
      ...
      if ( !bIsNotEmpty ) {
        std::cout << ( listVFATQualityFlag_[gemchId] == nullptr ? "WHAT" : "FUCK" ) << std::endl;
        edm::LogError("BadGeometry") << "Wrong detId which is not in the current geometry (VFAT status): "
          << gemchId << std::endl;
        continue;
      }
  ...
  for (GEMGEBdataCollection::DigiRangeIterator gebIt = gemGEB->begin(); gebIt != gemGEB->end(); ++gebIt) {
    ...
    if ( listTimeStore_.find((UInt_t)lid) == listTimeStore_.end() ) {
      edm::LogError("BadGeometry") << "Wrong detId which is not in the current geometry (GEB status): "
        << lid << std::endl;
      continue;
    }
